...
export default function PatientIntakeTableComponent({ sectionId }) {
  ...
  const fetchIntakeRecords = async () => {
    try {
      ...
      const patientData = await getPatientIntake(sectionPatientId);
  ...
  const handleSave = async () => {
    try {
      ...
      const parsedAmount = parseFloat(newIntake.amount);
      const typeValue =
        newIntake.type === "TUBE-FEEDING" ? "TUBE FEEDING" : newIntake.type;
  ...
  const handleDelete = async () => {
    ...
    setIntakes(intakes.filter((item) => item.id !== deletingRow.original.id));
