...
export async function getPatientMedicationById(section_patient_id) {
  try {
    const response = await axiosTokenInstance().get(
      `patient/${section_patient_id}/medication`
    );
    return response.data;
  } catch (error) {
    throw error;
...
export async function updatePatientMedication(section_patient_id, id, medication) {
  try {
...
export async function deletePatientMedication(section_patient_id, id, medication) {
  try {
